...
@Value
public class ComparableSchedule implements Iterable<ScheduledTask>, Comparable<ComparableSchedule> {

    private static final ComparableSchedule EMPTY = new ComparableSchedule();

    ComparableSchedule parent;
    ScheduledTask scheduledTask;
    int depth;
    int cost;

    private ComparableSchedule() {
        parent = null;
        scheduledTask = null;
        depth = 0;
    ...
    public ComparableSchedule(@NonNull ComparableSchedule parent, @NonNull ScheduledTask scheduledTask) {
        this.parent = parent;
    ...
    public static ComparableSchedule empty() {
        return EMPTY;
    }
    ...
    public Set<ComparableSchedule> expand(SchedulingContext context) {
        Set<ComparableSchedule> output = new HashSet<>();
        ...
        for(val node : context.getTaskGraph().getNodes()) {
            ...
            for(int i = 0; i < context.getProcessorCount(); ++i) {
                ...
                output.add(new ComparableSchedule(this, new ScheduledTask(i, startTime, node)));
            }
     ...
     */
    private ComparableSchedule findPartialScheduleFor(Node node) {
        var sched = this;
        while(!sched.isEmpty()) {
    ...
    public Iterator<ScheduledTask> iterator() {
        return new Iterator<ScheduledTask>() {
            private ComparableSchedule current = ComparableSchedule.this;
    ...
    @Override
    public int compareTo(ComparableSchedule o) {
